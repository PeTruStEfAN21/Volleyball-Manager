...
void MatchScreen::setAdversar(Echipeptr newOpponent) {
    if (newOpponent) {
        ...
        next_screen_id = MATCH_SCREEN;
    }
}

void MatchScreen::resetForNewSet() {
    echipaMea->resetPunctaj();
    echipaAdversa->resetPunctaj();
...
float MatchScreen::getWeightedTeamScore(Echipeptr echipa) const {
...
void MatchScreen::simulatePoint() {
    ...
    scorMea *= echipaMea->getMomentum();
    scorAdversa *= echipaAdversa->getMomentum();
...
void MatchScreen::finalizeMatch() {
...
void MatchScreen::handleInput(const sf::Event& event, sf::RenderWindow& window) {
...
void MatchScreen::render(sf::RenderWindow& window) {
...
void MatchScreen::resetare_scoruri() {
    echipaMea->resetPunctaj();
    echipaMea->resetSetur();
    echipaAdversa->resetPunctaj();
    echipaAdversa->resetSetur();
...
int MatchScreen::run(sf::RenderWindow& window) {
    ...
    while (window.isOpen() && next_screen_id == MATCH_SCREEN) {
